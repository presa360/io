...
(function () {
  ...
  function clampField(def, value, warnings) {
    if (def.type === 'text' || def.type === 'textarea') {
      const s = value || '';
      // Title and description (textarea) are NOT truncated any more — the renderer
      // autoscales them to fit. We only warn when they run long.
      const autoFit = def.type === 'textarea' || def.key === 'title';
