...
(function () {
  ...
  const normBrand = (b) => (!b || typeof b === 'string')
    ? { name: b || 'Presa', logo: null, logoRatio: null, pageNum: true, showName: true }
    : { name: b.name || 'Presa', logo: b.logo || null, logoRatio: b.logoRatio || null, pageNum: b.pageNum !== false, showName: b.showName !== false };
  ...
  function footer(pptx, slide, p, n, total, brand, onDark) {
    ...
    if (B.showName !== false) slide.addText(B.name, { x: 0.62, y: H - 0.68, w: 5, h: 0.36, fontFace: F, fontSize: 12, bold: true, color: onDark ? 'FFFFFF' : hex(p.title), valign: 'middle' });
  ...
  function iconUri(name, colorHex) {
    ...
    const ic = set[name] || set.spark || [];
    const fill = ic && ic.f;
    const paths = fill ? ic.p : ic;
    const vb = fill ? (ic.vb || '0 -960 960 960') : '0 0 24 24';
    const inner = paths.map((d) => `<path d="${d}"/>`).join('');
    const svg = fill
      ? `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="${vb}" fill="${colorHex}">${inner}</svg>`
      : `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="${vb}" fill="none" stroke="${colorHex}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
    return 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svg)));
  }

  // Native icon mark. We deliberately DON'T embed the SVG glyph as an image:
  // pptxgenjs rasterizes SVG images through a browser <canvas> (new Image() +
  // canvas.toDataURL), a fragile path that can silently corrupt the downloaded
  // file (tainted canvas / zero-sized SVG → the write promise rejects). A filled
  // rounded square in the icon colour is robust and matches the corporate style.
  function iconMark(pptx, slide, colorHex, x, y, w, h) {
    const s = Math.min(w, h);
    slide.addShape(pptx.ShapeType.roundRect, {
      x: x + (w - s) / 2, y: y + (h - s) / 2, w: s, h: s,
      rectRadius: s * 0.28, fill: { color: hex(colorHex) }, line: { type: 'none' }
    });
  }
  ...
  L.service_icons = (c) => {
    ...
    cards.forEach((card, i) => {
      ...
      iconMark(pptx, slide, '#FFFFFF', x + t.pad + t.badge * 0.22, y + t.pad + t.badge * 0.22, t.badge * 0.56, t.badge * 0.56);
  ...
  L.cta = (c) => {
    ...
    if (FB.showName !== false) slide.addText(FB.name, { x: ftx, y: H - 0.68, w: 3, h: 0.36, fontFace: F, fontSize: 10, bold: true, color: 'FFFFFF', valign: 'middle' });
  ...
  L.about_geo_icons = (c) => {
    ...
    cards.forEach((cd, i) => {
      const { x, w } = grid[i];
      iconMark(pptx, slide, p.accent, x, y, 0.62, 0.62);
  ...
  L.solution_badges = (c) => {
    ...
    cards.forEach((cd, i) => {
      ...
      iconMark(pptx, slide, p.accent, M + bSz * 0.24, y + bSz * 0.24, bSz * 0.52, bSz * 0.52);
  ...
  L.numbers_icon_stats = (c) => {
    ...
    stats.forEach((s, i) => {
      const { x, w } = grid[i];
      iconMark(pptx, slide, p.accent, x, y, 0.66, 0.66);
  ...
  const MAP = {
    ...
    // pack 2 — data-viz & structure → nearest native PPTX composition
    kpi_dashboard: (ctx) => L.stats_row({ ...ctx, f: { ...ctx.f, stats: (ctx.f.stats || []).map((s) => ({ value: s.value, label: [s.label, s.delta].filter(Boolean).join('  ') })) } }),
    stat_hero: (ctx) => L.big_stat({ ...ctx, f: { ...ctx.f, stat: [ctx.f.value, ctx.f.unit].filter(Boolean).join(''), label: ctx.f.caption, bullets: (ctx.f.points || []).map((x) => [x.heading, x.text].filter(Boolean).join(' — ')) } }),
    bar_compare: (ctx) => L.chart_bar({ ...ctx, f: { ...ctx.f, data: ctx.f.data } }),
    ring_kpis: (ctx) => L.stats_row({ ...ctx, f: { ...ctx.f, stats: (ctx.f.stats || []).map((s) => ({ value: s.value, label: s.label })) } }),
    feature_matrix: (ctx) => L.table({ ...ctx, f: { ...ctx.f, headers: [ctx.f.rowsLabel || ''].concat(ctx.f.columns || []), rows: (ctx.f.rows || []).map((r) => ({ c1: r.feature, c2: r.c1, c3: r.c2, c4: r.c3 })) } }),
    section_divider: (ctx) => L.title_dark({ ...ctx, f: { ...ctx.f, title: [ctx.f.num, ctx.f.title].filter(Boolean).join('  '), subtitle: ctx.f.subtitle } }),
    // pack 3 — Sprint 1: tables & pricing → nearest native PPTX composition
    feature_matrix_priced: (ctx) => L.table({ ...ctx, f: { ...ctx.f, h1: ctx.f.rowsLabel || '', h2: [(ctx.f.columns || [])[0], (ctx.f.prices || [])[0]].filter(Boolean).join(' '), h3: [(ctx.f.columns || [])[1], (ctx.f.prices || [])[1]].filter(Boolean).join(' '), rows: (ctx.f.rows || []).map((r) => ({ c1: r.feature, c2: r.c1, c3: r.c2 })) } }),
    compare_check: (ctx) => L.table({ ...ctx, f: { ...ctx.f, h1: ctx.f.rowsLabel || '', h2: (ctx.f.columns || [])[0] || '', h3: (ctx.f.columns || [])[1] || '', rows: (ctx.f.rows || []).map((r) => ({ c1: r.feature, c2: r.c1, c3: r.c2 })) } }),
    price_matrix: (ctx) => L.table({ ...ctx, f: { ...ctx.f, h1: ctx.f.rowsLabel || '', h2: [(ctx.f.columns || [])[0], (ctx.f.prices || [])[0]].filter(Boolean).join(' '), h3: [(ctx.f.columns || [])[1], (ctx.f.prices || [])[1]].filter(Boolean).join(' '), rows: (ctx.f.rows || []).map((r) => ({ c1: r.feature, c2: r.c1, c3: r.c2 })) } }),
    role_matrix: (ctx) => L.table({ ...ctx, f: { ...ctx.f, h1: ctx.f.rowsLabel || '', h2: (ctx.f.columns || [])[0] || '', h3: (ctx.f.columns || [])[1] || '', rows: (ctx.f.rows || []).map((r) => ({ c1: r.feature, c2: r.c1, c3: r.c2 })) } }),
    data_total: (ctx) => L.table({ ...ctx, f: { ...ctx.f, h1: ctx.f.h1 || 'Статья', h2: ctx.f.h3 || 'Стало', h3: ctx.f.h4 || 'Разница', rows: (ctx.f.rows || []).map((r) => ({ c1: r.label, c2: r.c2, c3: r.c3 })).concat((ctx.f.totalLabel || ctx.f.totalValue) ? [{ c1: ctx.f.totalLabel || 'Итого', c2: '', c3: ctx.f.totalValue || '' }] : []) } }),
    bar_table: (ctx) => L.chart_bar({ ...ctx, f: { ...ctx.f, data: (ctx.f.data || []).map((d) => ({ label: d.label, value: (d.pct != null && d.pct !== '') ? d.pct : d.value })), note: ctx.f.subtitle } }),
    tier_3_featured: L.pricing,
    tier_2_big: (ctx) => L.pricing({ ...ctx, f: { ...ctx.f, tiers: (ctx.f.tiers || []).map((t) => ({ name: t.name, price: t.price, features: t.features })) } }),
    single_price: (ctx) => L.big_stat({ ...ctx, f: { ...ctx.f, statValue: ctx.f.price, statLabel: ctx.f.per, bullets: (ctx.f.features || []).slice(0, 3) } }),
    value_scale: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: (ctx.f.steps || []).map((s) => ({ title: [s.price, s.name].filter(Boolean).join(' · '), desc: s.desc })) } }),
    // pack 3 — Sprint 2: case studies & comparison → nearest native PPTX composition
    before_after: (ctx) => L.text_stats({ ...ctx, f: { ...ctx.f, text: [(ctx.f.beforeLabel || 'Было') + ': ' + (ctx.f.before || []).join('; '), (ctx.f.afterLabel || 'Стало') + ': ' + (ctx.f.after || []).join('; ')].join('\n'), stats: ctx.f.kpis } }),
    quote_result: (ctx) => L.text_stats({ ...ctx, f: { ...ctx.f, title: '\u201C' + (ctx.f.quote || '') + '\u201D', text: [ctx.f.author, ctx.f.role].filter(Boolean).join(' · '), stats: ctx.f.stats } }),
    case_timeline: (ctx) => L.timeline({ ...ctx, f: { ...ctx.f, milestones: (ctx.f.milestones || []).map((m, i, a) => ({ title: m.week, desc: (i === a.length - 1 && ctx.f.resultValue) ? ctx.f.resultValue : (m.title + (m.desc ? ' — ' + m.desc : '')) })) } }),
    case_result_photo: (ctx) => L.text_stats({ ...ctx, f: { ...ctx.f, text: ['Задача: ' + (ctx.f.challenge || ''), 'Решение: ' + (ctx.f.solution || '')].join('\n'), stats: ctx.f.kpis } }),
    vs_columns: (ctx) => L.compare({ ...ctx, f: { ...ctx.f, leftLabel: ctx.f.winLabel, rightLabel: ctx.f.loseLabel, rows: (ctx.f.winItems || []).map((w, i) => ({ label: '', left: w, right: (ctx.f.loseItems || [])[i] || '' })) } }),
    transformation: (ctx) => L.text_stats({ ...ctx, f: { ...ctx.f, text: [(ctx.f.beforeTitle || '') + ' — ' + (ctx.f.beforeText || ''), '→ ' + (ctx.f.afterTitle || '') + ' — ' + (ctx.f.afterText || '')].join('\n'), stats: ctx.f.kpis } }),
    criteria_scale: (ctx) => L.compare({ ...ctx, f: { ...ctx.f, leftLabel: ctx.f.usLabel || 'Presa', rightLabel: ctx.f.themLabel || 'Другие', rows: (ctx.f.rows || []).map((r) => ({ label: r.label, left: r.us, right: r.them })) } }),
    // pack 3 — Sprint 4: charts → nearest native PPTX composition
    chart_donut2: L.chart_donut,
    bar_horizontal: L.chart_bar,
    chart_trend: (ctx) => L.chart_line({ ...ctx, f: { ...ctx.f, note: ctx.f.delta } }),
    // pack 3 — Sprint 4: contact → nearest native PPTX composition
    cta_qr: (ctx) => L.cta({ ...ctx, f: { ...ctx.f, bullets: [ctx.f.buttonLabel, ctx.f.qrCaption].filter(Boolean) } }),
    contact_map: (ctx) => L.cta({ ...ctx, f: { ...ctx.f, subtitle: ctx.f.city, bullets: (ctx.f.items || []).map((it) => [it.label, it.value].filter(Boolean).join(': ')) } }),
    cta_accent: (ctx) => L.title_dark({ ...ctx, f: { ...ctx.f, subtitle: ctx.f.subtitle, footerLabel: (ctx.f.chips || []).join('  ·  ') } }),
    // pack 3 — Sprint 3: roadmap / team / process → nearest native PPTX composition
    roadmap_quarters: (ctx) => L.timeline({ ...ctx, f: { ...ctx.f, milestones: (ctx.f.cols || []).map((c) => ({ title: [c.q, c.tag].filter(Boolean).join(' · '), desc: (c.items || []).join('; ') })) } }),
    roadmap_milestones: (ctx) => L.timeline({ ...ctx, f: { ...ctx.f, milestones: (ctx.f.milestones || []).map((m) => ({ title: [m.w, m.heading].filter(Boolean).join(' · '), desc: m.text })) } }),
    roadmap_gantt: (ctx) => L.timeline({ ...ctx, f: { ...ctx.f, milestones: (ctx.f.rows || []).map((r) => ({ title: r.label, desc: r.note })) } }),
    roadmap_phases: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: (ctx.f.phases || []).map((ph) => ({ title: [ph.lab, ph.heading].filter(Boolean).join(' · '), desc: (ph.items || []).join('; ') })) } }),
    team_grid_6: (ctx) => L.team({ ...ctx, f: { ...ctx.f, members: ctx.f.members } }),
    team_lead: (ctx) => L.team({ ...ctx, f: { ...ctx.f, members: [{ name: ctx.f.leadName, role: ctx.f.leadRole }].concat(ctx.f.members || []) } }),
    team_quote_split: (ctx) => L.team({ ...ctx, f: { ...ctx.f, title: '\u201C' + (ctx.f.quote || '') + '\u201D', members: ctx.f.members } }),
    team_trust: (ctx) => L.team({ ...ctx, f: { ...ctx.f, members: ctx.f.members } }),
    process_arrows: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: ctx.f.steps } }),
    process_cycle: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: (ctx.f.steps || []).map((s) => ({ title: s.heading, desc: s.text })) } }),
    process_owners: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: (ctx.f.steps || []).map((s) => ({ title: s.title, desc: [s.desc, s.owner ? '— ' + s.owner + (s.ownerRole ? ' (' + s.ownerRole + ')' : '') : ''].filter(Boolean).join(' ') })) } }),
    // pack 3 — Sprint 4: numbers & solution → nearest native PPTX composition
    progress_rings: (ctx) => L.stats_row({ ...ctx, f: { ...ctx.f, stats: (ctx.f.rings || []).map((r) => ({ value: (r.pct || '') + '%', label: [r.heading, r.desc].filter(Boolean).join(' — ') })) } }),
    solution_flow: (ctx) => L.text_stats({ ...ctx, f: { ...ctx.f, text: [(ctx.f.inputsLabel || 'Было') + ': ' + (ctx.f.inputs || []).map((i) => i.text).join('; '), (ctx.f.outputsLabel || 'Стало') + ': ' + (ctx.f.outputs || []).map((o) => o.text).join('; ')].join('\n') } }),
    solution_layers: (ctx) => L.steps({ ...ctx, f: { ...ctx.f, steps: (ctx.f.layers || []).map((ly) => ({ title: ly.heading, desc: ly.text })) } }),
