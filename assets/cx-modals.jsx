...
(function () {
  ...
  /* Raw text → semantic content object (title/text/bullets/cards/stats…).
     remapFields then lays it onto whatever fields the chosen template exposes,
     so one pasted blurb fills exactly one slide in its design. */
  function contentFromText(raw) {
    const lines = String(raw || '').replace(/\r/g, '').split('\n').map((s) => s.trim()).filter(Boolean);
    if (!lines.length) return {};
    const strip = (s) => s.replace(/^#{1,3}\s*/, '').replace(/^\d{1,2}[.)]\s*/, '').replace(/^[-—–•*·▪►▶]\s*/, '').trim();
    const title = strip(lines[0]).slice(0, 80);
    const rest = lines.slice(1);
    const isList = (s) => /^[-—–•*·▪►▶]/.test(s) || (s.length <= 64 && s.split(/\s+/).length <= 9 && !/[.!?…]$/.test(s));
    const prose = [], bullets = [];
    rest.forEach((s) => { (isList(s) ? bullets : prose).push(strip(s)); });
    const metrics = (window.CXWiz && window.CXWiz.extractMetrics) ? window.CXWiz.extractMetrics(raw) : [];
    const out = { title };
    const lead = prose.length ? prose.join(' ') : (bullets[0] || '');
    if (lead) { out.text = prose.length ? prose.join(' ') : ''; out.intro = out.text; out.subtitle = (prose[0] || bullets[0] || '').slice(0, 140); out.quote = lead; }
    if (bullets.length) {
      const b = bullets.slice(0, 8);
      out.bullets = b; out.items = b;
      const cards = b.map((x) => { const m = x.split(/\s*[—:–]\s*/); return { heading: (m[0] || x).slice(0, 40), text: m.slice(1).join(' ') }; });
      out.cards = cards; out.steps = cards.map((c) => ({ title: c.heading, desc: c.text }));
      out.members = b.map((n) => ({ name: n, role: '' }));
    }
    if (metrics.length >= 2) out.stats = metrics.slice(0, 4);
    if (metrics.length) { out.statValue = metrics[0].value; out.statLabel = metrics[0].label; }
    return out;
  }
  ...
  function AddSlideModal({ theme, onAdd, onClose }) {
    ...
    const [aiTpl, setAiTpl] = useState(null);   // template being AI-filled
    const [aiText, setAiText] = useState('');
    const aiFields = useMemo(() => (aiTpl && aiText.trim()) ? window.CX.remapFields(aiTpl, contentFromText(aiText)) : null, [aiTpl, aiText]);
    const closeAi = () => { setAiTpl(null); setAiText(''); };
    ...
    return h('div', { className: 'cx-overlay', onMouseDown: (e) => { if (e.target === e.currentTarget) onClose(); } },
      h('div', { className: 'cx-libmodal', 'data-screen-label': 'Библиотека шаблонов' },
              ...
              results.map((t) => {
                ...
                return h('div', { key: t.id, className: 'cx-tplcard' },
                  h('div', { className: 'cx-tplcard__pic' },
                    ...
                    h('div', { className: 'cx-tplcard__actions' },
                      h('button', { className: 'cx-tplcard__btn', onClick: () => onAdd(t) }, h(Svg, { d: I.plus, s: 14 }), 'Добавить'),
                      h('button', { className: 'cx-tplcard__btn cx-tplcard__btn--ai', title: 'Вставить текст — ИИ заполнит этот слайд', onClick: () => { setAiTpl(t); setAiText(''); } }, h(Svg, { d: I.spark, s: 14 }), 'AI'))),
        ...
        aiTpl ? h('div', { className: 'cx-aicompose', onMouseDown: (e) => { if (e.target === e.currentTarget) closeAi(); } },
          h('div', { className: 'cx-aicompose__card', 'data-screen-label': 'AI-заполнение слайда' },
            h('div', { className: 'cx-aicompose__head' },
              h('span', { className: 'cx-aicompose__ic' }, h(Svg, { d: I.spark, s: 16 })),
              h('div', { className: 'cx-aicompose__tx' }, h('b', null, 'Заполнить слайд по тексту'), h('span', null, aiTpl.name)),
              h('button', { className: 'cx-modal__close', onClick: closeAi }, h(Svg, { d: I.close, s: 15 }))),
            h('div', { className: 'cx-aicompose__prev' },
              h(window.CX.MiniSlide, { template: aiTpl, fields: aiFields || aiTpl.sample || {}, theme, n: 1, total: 1 })),
            h('textarea', { className: 'fe-input fe-textarea cx-aicompose__area', autoFocus: true, rows: 5,
              placeholder: 'Вставьте текст для этого слайда — заголовок, описание, пункты, цифры. ИИ сам разложит его по полям выбранного дизайна.',
              value: aiText, onChange: (e) => setAiText(e.target.value) }),
            h('div', { className: 'cx-aicompose__row' },
              h('span', { className: 'cx-aicompose__hint' }, aiText.trim() ? 'Превью обновляется слева' : 'Превью появится по мере ввода'),
              h('div', { className: 'cx-aicompose__btns' },
                h('button', { className: 'btn btn-ghost btn-sm', onClick: closeAi }, 'Отмена'),
                h('button', { className: 'btn btn-primary btn-sm', disabled: !aiText.trim(), onClick: () => { onAdd(aiTpl, aiFields || undefined); closeAi(); } },
                  h(Svg, { d: I.spark, s: 14 }), 'Создать слайд'))))) : null));
  }
