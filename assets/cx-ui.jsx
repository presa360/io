...
(function () {
  ...
  // Strategy: (1) exact-key carry, (2) semantic carry between compatible shapes
  // (list↔group, text↔intro↔subtitle, title↔heading), (3) sample fallback.
  // Finally, ANY field the user filled that the new template doesn't use is kept
  // untouched, so switching variants back and forth never loses content.
  function remapFields(tpl, oldFields) {
    const out = {};
    const old = oldFields || {};
    const defs = [...tpl.requiredFields, ...(tpl.optionalFields || [])];
    const isEmpty = (v) => v === undefined || v === null || v === '' || (Array.isArray(v) && v.length === 0);
    const has = (k) => !isEmpty(old[k]);

    const LISTKEYS = ['bullets', 'items'];
    const GROUPKEYS = ['cards', 'steps', 'members', 'milestones', 'projects', 'rows'];
    const TEXTKEYS = ['text', 'intro', 'subtitle', 'quote'];
    const HEADKEYS = ['title', 'heading'];
    const firstWith = (keys, exclude) => keys.find((k) => k !== exclude && has(k));
    const itemHead = (it) => typeof it === 'string' ? it : (it && (it.heading || it.title || it.name || it.label)) || '';

    // shape an old card/bullet into the target group's sub-fields
    function coerceItem(subDefs, src) {
      const o = {};
      const isStr = typeof src === 'string';
      const s = src || {};
      const textSubs = subDefs.filter((d) => d.type === 'text' || d.type === 'textarea');
      let head, desc;
      if (isStr) {
        // "Метка: описание" / "Метка — описание" → heading + text, so switching a
        // bullet slide to a card variant fills BOTH fields (not only the heading).
        // Only when the target card actually has a second text slot to hold it.
        const m = textSubs.length >= 2 ? String(src).match(/^\s*(.{2,44}?)\s*[:：—–]\s+(\S.+)$/) : null;
        if (m && m[1].trim().split(/\s+/).length <= 7) { head = m[1].trim(); desc = m[2].trim(); }
        else { head = src; desc = ''; }
      } else {
        head = s.heading || s.title || s.name || s.label || '';
        desc = s.text || s.desc || s.role || s.subtitle || '';
      }
      subDefs.forEach((sd) => {
        if (!isStr && s[sd.key] !== undefined && s[sd.key] !== '') { o[sd.key] = s[sd.key]; return; }
        if (sd === textSubs[0]) o[sd.key] = head;
        else if (sd === textSubs[1]) o[sd.key] = desc;
        else o[sd.key] = (sd.type === 'list' || sd.type === 'group') ? [] : '';
      });
      return o;
    }
    ...
    defs.forEach((def) => {
      ...
      // (1) exact-key carry
      if (has(k)) {
        out[k] = (def.type === 'group' && Array.isArray(old[k])) ? old[k].map((it) => coerceItem(def.sub, it)) : old[k];
        return;
      }
      // (2) semantic carry from a compatible old field
      let val;
      if (def.type === 'list') {
        const lk = firstWith(LISTKEYS, k);
        if (lk) val = old[lk].slice();
        else { const gk = firstWith(GROUPKEYS); if (gk) val = old[gk].map(itemHead).filter(Boolean); }
      } else if (def.type === 'group') {
        const gk = firstWith(GROUPKEYS, k);
        if (gk) val = old[gk].map((it) => coerceItem(def.sub, it));
        else { const lk = firstWith(LISTKEYS); if (lk) val = old[lk].map((t) => coerceItem(def.sub, t)); }
      } else if (def.type === 'text' || def.type === 'textarea') {
        if (HEADKEYS.indexOf(k) >= 0) { const hk = firstWith(HEADKEYS, k); if (hk) val = old[hk]; }
        else if (TEXTKEYS.indexOf(k) >= 0) { const tk = firstWith(TEXTKEYS, k); if (tk) val = old[tk]; }
      }
      if (!isEmpty(val)) { out[k] = val; return; }
      // (3) sample fallback, then empty
      if (tpl.sample && tpl.sample[k] !== undefined) out[k] = JSON.parse(JSON.stringify(tpl.sample[k]));
      else out[k] = def.type === 'list' || def.type === 'group' ? [] : '';
    });

    // (4) keep any user content the new template doesn't expose, so toggling
    // variants back and forth never discards it.
    Object.keys(old).forEach((k) => { if (!(k in out) && !isEmpty(old[k])) out[k] = old[k]; });
    return out;
  }
